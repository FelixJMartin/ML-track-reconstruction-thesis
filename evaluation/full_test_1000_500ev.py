# ================================================================================
# pipeline_eval.py  — full dataset, fixed baseline + alpha=0.85
# ================================================================================
...
model_s1.load_state_dict(
    torch.load(os.path.join(Save_dir, "grid_s1", "s1_h512_256_lr0.001_dr0.1.pt"),
               map_location=torch.device('cpu'))["model_state"]
)
model_s1.eval(); model_s1.set_bs(1)

model_s2 = PhiFinderSNN(h1=512, h2=256, bs=1)
model_s2.load_state_dict(
    torch.load(os.path.join(Save_dir, "grid_s2", "s2_h512_256_lr0.001_dr0.1_pw10.0.pt"),
               map_location=torch.device('cpu'))["model_state"]
)
