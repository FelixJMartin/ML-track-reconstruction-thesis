# ================================================================================
# pipeline_eval.py  — 1000-event set, fixed baseline + alpha=0.8
# ================================================================================
...
# ── LOAD MODELS ───────────────────────────────────────────────────────────────
#baseline
# S1_MODEL = "model_stage1.pt"
# S2_MODEL = "stage2_s2_h512_256_lr1e-3_dr0.1_pw10.pt"


# #best
S1_MODEL = "grid_s1/s1_h512_128_lr0.001_dr0.1.pt"
S2_MODEL = "grid_s2/s2_h512_256_lr0.001_dr0.1_pw10.0.pt"

print("Loading models...")

ckpt_s1  = torch.load(os.path.join(Save_dir, S1_MODEL), map_location='cpu')
cfg_s1   = ckpt_s1['config']
model_s1 = ConeFilterSNN(cfg_s1['SNN_R_BINS'], h1=cfg_s1['H1'], h2=cfg_s1['H2'], bs=1)
model_s1.load_state_dict(ckpt_s1["model_state"])
model_s1.eval(); model_s1.set_bs(1)

ckpt_s2  = torch.load(os.path.join(Save_dir, S2_MODEL), map_location='cpu')
cfg_s2   = ckpt_s2['config']
model_s2 = PhiFinderSNN(h1=cfg_s2['H1'], h2=cfg_s2['H2'], bs=1)
...
s2_eff   = s2_detected / s2_total_sig if s2_total_sig > 0 else 0
...
print(f"{'═'*60}")
print(f"  Hit reduction:")
print(f"    Baseline : {hits_baseline_all.mean():>10,.0f} hits/event")
...
print(f"  Stage 2: efficiency={s2_eff*100:.1f}%  precision={s2_prec*100:.1f}%")
print(f"  Track survival: {full_eff*100:.1f}%  ({tracks_survived}/{tracks_total})")
print(f"{'═'*60}")

# ── PLOT 1: pipeline summary ───────────────────────────────────────────────────
fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(24, 5))

x = np.arange(len(hits_baseline_all))
ax0.bar(x - 0.25, hits_baseline_all / 1000, width=0.25, color=C[0], alpha=0.85, label='Pipeline input')
ax0.bar(x,        hits_after_s1_all / 1000, width=0.25, color=C[1], alpha=0.85, label='After Stage 1')
ax0.bar(x + 0.25, hits_after_s2_all / 1000, width=0.25, color=C[2], alpha=0.85, label='After Stage 2')
ax0.set_xlabel('Event index')
ax0.set_ylabel('Hits (thousands)', labelpad=10)

legend = ax0.legend(fontsize=9, loc='upper right', bbox_to_anchor=(0.99, 0.99))
legend.get_frame().set_linewidth(0.6)

ax0.set_xticks(x)
ax0.set_xticklabels([str(i) for i in range(len(hits_baseline_all))], fontsize=7)
ax0.grid(axis='y', color='grey', alpha=0.3, linewidth=0.5)
ax0.set_axisbelow(True)

metrics = ['Track survival', 'S2 precision', 'S2 efficiency', 'S1 throwrate', 'S1 efficiency']
values  = [full_eff*100, s2_prec*100, s2_eff*100, s1_throw*100, s1_eff*100]
colors  = [C[2], C[0], C[1], C[0], C[1]]
bars    = ax1.barh(metrics, values, color=colors, alpha=0.85, height=0.5)
ax1.set_xlim(0, 115)
ax1.set_xlabel('%', labelpad=10)
ax1.grid(axis='x', color='grey', alpha=0.3, linewidth=0.5)
