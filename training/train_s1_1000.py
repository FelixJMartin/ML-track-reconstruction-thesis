...
# ── IMPORTS FROM build_data ───────────────────────────────────────────────────
from build_data1000 import (make_rz_grid, get_cone, CONE_SIGNS, SAMPLE_FRACTION,
...
class ConeFilterSNN(nn.Module):
    """
    Binary classifier: does this cone contain a track?
    Input:  (batch, 100, 100) r-z matrix
    Output: (batch,) logit — sigmoid gives P(track)
    """
...
if __name__ == "__main__":
    ...
    if os.path.exists(model_path):
        ...
        trained_model.eval()
        train_losses = checkpoint['train_losses']  # add these
    ...
    # test on held-out event outside training range (1000-2799)
    test_event = os.path.join(data_dir, "event000002800")
    scan_event(test_event, trained_model, threshold=0.7)

    y = np.load(os.path.join(Save_dir, "y_stage1.npy"))
    n_pos = y.sum()
    n_neg = len(y) - n_pos
    print(f"ratio: {n_neg/n_pos:.1f}")