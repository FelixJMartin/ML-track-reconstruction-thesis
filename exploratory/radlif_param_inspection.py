...
Save_dir_data = os.path.join(Save_dir, "train")
...
# ── Load data ─────────────────────────────────────────────────────────────────
X = np.load(os.path.join(Save_dir_data, "X_rz.npy"))
y = np.load(os.path.join(Save_dir_data, "y_stage1.npy"))

# ── 1. Learned parameter distributions ───────────────────────────────────────
params_to_plot = {
    'lif1.alpha': r'membrane decay $\alpha$',
    'lif1.beta':  r'adapt. decay $\beta$',
    'lif1.a':     r'coupling $a$',
    'lif1.b':     r'spike adapt. $b$',
    'lif2.alpha': r'membrane decay $\alpha$',
    'lif2.beta':  r'adapt. decay $\beta$',
    'lif2.a':     r'coupling $a$',
    'lif2.b':     r'spike adapt. $b$',
}

fig, axes = plt.subplots(2, 4, figsize=(14, 5))
fig.subplots_adjust(left=0.07)

for i, (ax, (key, title)) in enumerate(zip(axes.flat, params_to_plot.items())):
    values = state[key].numpy()
    ax.hist(values, bins=30, color='steelblue', alpha=0.7, edgecolor="white", linewidth=0.4)
    ax.axvline(values.mean(),     color='black', linestyle="--", linewidth=1.2,
               label=f"mean={values.mean():.3f}")
    ax.axvline(np.median(values), color='tomato', linestyle=":",  linewidth=1.2,
...
fig.text(0.015, 0.72, 'Layer 1', va='center', rotation='vertical', fontsize=9, fontweight='bold')
fig.text(0.015, 0.28, 'Layer 2', va='center', rotation='vertical', fontsize=9, fontweight='bold')
plt.tight_layout()
plt.show()
