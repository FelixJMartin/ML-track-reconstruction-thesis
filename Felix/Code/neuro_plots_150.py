...
sys.path.append(r"C:\Users\felix\Downloads\Skola\Kand\BACHELOR_CODE\Felix\Code")
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

from trackml.dataset import load_event
from Functions import preprocess_particles
from Build_datamax import make_rz_grid, get_cone, CONE_SIGNS
...
for col, idx in enumerate(show_sig):
    ...
    ax.set_title(f'Signal #{idx}', fontsize=8)
...
for col, idx in enumerate(show_noi):
    ...
    ax.imshow(X_rz[idx].T, origin='lower', aspect='auto', cmap='Oranges',
              extent=[0, 1100, 0, 3000])
    ax.set_title(f'Noise #{idx}', fontsize=8)
    ax.set_xlabel('r  [mm]', fontsize=7)
    if col == 0:
        ax.set_ylabel('|z|  [mm]', fontsize=7)
    else:
...
plt.suptitle('Example r-z grids  (5% noise, 150×150)', fontsize=10, y=1.01)
plt.tight_layout()
plt.show()
