# ================================================================================
# stage2_phi_localiser.py  — 1000 event pipeline
# ================================================================================
    ...
    if plot:
        ...
        if plottable:
            fig, axes = plt.subplots(1, len(plottable),
                                     figsize=(len(plottable) * 3.5, 3.5),
                                     squeeze=False)
            for ax, r in zip(axes[0], plottable):
                sx, sy, sz = r['cone']
                bars = ax.bar(range(SNN_PHI_BINS), r['phi_probs'],
                              width=1, color='#cce0f5', linewidth=0)
                for b in r['active_bins']:
                    bars[b].set_color('#1a6bb5')
                for b in r['true_bins']:
                    ax.axvline(b, color='#c0392b', lw=0.8, alpha=0.7)
                ax.axhline(thresh_s2, color='gray', lw=0.8, ls='--')
                overlap = len(set(r['active_bins']) & set(r['true_bins']))
                ax.set_title(f"cone({sx:+d},{sy:+d},{sz:+d})\n"
                             f"s1={r['s1_prob']*100:.0f}%  "
                             f"pred={len(r['active_bins'])}  "
                             f"true={len(r['true_bins'])}  "
                             f"overlap={overlap}", fontsize=8)
                ax.set_xlabel("phi bin", fontsize=8)
                ax.set_ylabel("P(track)", fontsize=8)
                ax.set_ylim(0, 1.05)
            plt.suptitle(
                f"Stage 2 phi predictions — {os.path.basename(event_path)}\n"
                f"blue=predicted  |  red lines=true track bins", fontsize=9)
            plt.tight_layout()
            plt.show()
...
if __name__ == "__main__":
    ...
    if os.path.exists(s2_input_path):
        ...
        model_s1_filter = ConeFilterSNN(SNN_R_BINS, bs=1)
        model_s1_filter.load_state_dict(
            torch.load(os.path.join(Save_dir, "model_stage1.pt"))['model_state']
        )
        model_s1_filter.eval(); model_s1_filter.set_bs(1)

        probs = []
        with torch.no_grad():
            for i in range(len(X_rz)):
                p = torch.sigmoid(
                    model_s1_filter(torch.tensor(X_rz[i]).unsqueeze(0))
                ).item()
                probs.append(p)
                if i % 500 == 0:
                    print(f"  {i}/{len(X_rz)}...")
        ...
        np.save(s2_input_path, X_rphi_s2)
        np.save(s2_label_path, y_s2_filt)
        n_fp = (1 - y_s1[passed_mask]).sum()
        print(f"Stage 1 passed {passed_mask.sum()}/{len(X_rz)}  "
              f"contamination={100*n_fp/passed_mask.sum():.1f}%")
    ...
    X_rphi_s2_train = X_rphi_s2
    y_s2_train      = y_s2_filt
    ...
    test_event = os.path.join(data_dir, "event000002800")
