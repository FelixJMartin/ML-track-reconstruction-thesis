...
def histogram_of_particle_momenta(particles):
    ...
    ax.axvline(x=1.0, color='black', linestyle='--', linewidth=1.2,
               label=r'Signal cut ($p_T > 10\,\mathrm{GeV}/c$)')
    ax.set_xlabel(r'$\log_{10}$ Momentum (GeV/$c$)')
...
def plot_spike_full(spike_matrix, spike_row, cur, mem, spk, thr_line=1,
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[2.5, 1.5], wspace=0.2)
    ...
    gs_r = gs[1].subgridspec(3, 1, height_ratios=[1, 1, 0.4], hspace=0.05)
...
def plot_hits(hits):
    ...
    ax.legend(markerscale=5, fontsize=8, loc='upper left', frameon=False)
