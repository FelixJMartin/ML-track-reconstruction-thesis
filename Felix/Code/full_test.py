...
plt.rcParams.update({
    ...
    "axes.grid": True, "grid.alpha": 0.3, "grid.color": "#aaaaaa",
    "grid.linestyle": "--", "grid.linewidth": 0.6,
    "legend.framealpha": 0.3, "legend.edgecolor": "#aaaaaa", "legend.facecolor": "white",
    "legend.fontsize": 9,
})
...
Save_dir  = r"C:\Users\felix\Downloads\Skola\Kand\BACHELOR_CODE\data\models"
...
model_s1.load_state_dict(
    torch.load(os.path.join(Save_dir, "grid_s1", "s1_h512_256_lr0.001_dr0.1.pt"), map_location=torch.device('cpu'))["model_state"]
)
model_s1.eval(); model_s1.set_bs(1)

model_s2 = PhiFinderSNN(h1=512, h2=256, bs=1)
model_s2.load_state_dict(
    torch.load(os.path.join(Save_dir, "grid_s2", "s2_h512_256_lr0.001_dr0.1_pw10.0.pt"), map_location=torch.device('cpu'))["model_state"]
)
...
ax0.bar(x - 0.25, hits_baseline_all / 1000,  width=0.25, color=C[0], alpha=0.85, label='Pipeline input', zorder=3)
ax0.bar(x,        hits_after_s1_all / 1000,  width=0.25, color=C[1], alpha=0.85, label='After stage 1',  zorder=3)
ax0.bar(x + 0.25, hits_after_s2_all / 1000,  width=0.25, color=C[2], alpha=0.85, label='After stage 2',  zorder=3)
...
leg = ax0.legend(loc='upper right', frameon=True, framealpha=0.7,
                 facecolor='white', edgecolor='#888888', borderpad=0.8)
...
bars    = ax1.barh(metrics, values, color=colors, alpha=0.85, zorder=3)
ax1.set_xlim(0, 110)
ax1.set_xlabel('%')
