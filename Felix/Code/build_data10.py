...
def build_dataset(data_dir, event_ids, save_dir, sample_fraction):
    """
    X_rz / y_stage1 : built WITH augmentation (N_AUGMENT shuffles per track).
    X_rphi / y_stage2: built WITHOUT augmentation (one matrix per track).
    The two arrays have different lengths and are used independently.
    """
    X_rz_all       = []
    y_rz_all       = []
    X_rphi_all     = []
        ...
        try:
            ...
            for sx, sy, sz in CONE_SIGNS:
                ...
                for pid in sig['particle_id'].unique():
                    ...
                    # ── rphi — NO augmentation (one entry per track) ──────────
                    all_r_phi = np.concatenate([track['r'].values,   noi['r'].values])
                    all_phi   = np.concatenate([track['phi'].values, noi['phi'].values])
                    X_rphi_all.append(make_rphi_grid(all_r_phi, all_phi, sx, sy))
                    X_rphi_sig_all.append(make_rphi_grid(
                        track['r'].values, track['phi'].values, sx, sy
                    ))

                    X_rphi_all.append(make_rphi_grid(noi['r'].values, noi['phi'].values, sx, sy))
                    X_rphi_sig_all.append(np.zeros((SNN_R_BINS, SNN_PHI_BINS), dtype=np.float32))

                    # ── rz — WITH augmentation (N_AUGMENT shuffles) ───────────
            ...
            print(f"  {event_id}: tracks={total}  rz={len(X_rz_all)}  rphi={len(X_rphi_all)}")
    ...
    y_stage1   = np.array(y_rz_all, dtype=np.float32)

    X_rphi     = np.stack(X_rphi_all)
    X_rphi_sig = np.stack(X_rphi_sig_all)
    y_stage2   = (X_rphi_sig.sum(axis=1) > 0).astype(np.float32)
    ...
    print(f"\nSaved to {save_dir}")
    print(f"  X_rz shape:   {X_rz.shape}  (augmented x{N_AUGMENT})")
    print(f"  X_rphi shape: {X_rphi.shape}  (not augmented)")
    print(f"  y_stage1: {y_stage1.shape}  signal={int(y_stage1.sum())}  noise={int((1-y_stage1).sum())}")
    print(f"  y_stage2: {y_stage2.shape}  mean active bins={y_stage2.sum(axis=1).mean():.1f}")
