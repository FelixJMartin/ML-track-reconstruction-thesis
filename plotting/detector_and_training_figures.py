...
DIR_150      = os.path.join(BASE_DIR, "matrices150")
DIR_1000     = os.path.join(BASE_DIR, "matrices1000")
OUT_DIR      = r"C:\Users\felix\Downloads\Skola\Kand\BACHELOR_CODE\figures"
os.makedirs(OUT_DIR, exist_ok=True)

# ── Model paths ───────────────────────────────────────────────────────────────
S2_100_PATH  = os.path.join(BASE_DIR, "models", "grid_s2", "s2_h512_256_lr0.0005_dr0.1_pw10.0.pt")
...
def plot_3d_octants(hits, s=1):
    ...
    ax.set_xlim(-1200, 1200); ax.set_ylim(-1200, 1200); ax.set_zlim(-1200, 1200)
    ax.set_xlabel('Z (mm)', labelpad=8, color='#aaaaaa', fontsize=8)
    ax.set_ylabel('X (mm)', labelpad=8, color='#aaaaaa', fontsize=8)
    ax.set_zlabel('Y (mm)', labelpad=8, color='#aaaaaa', fontsize=8)
    ax.tick_params(colors='#cccccc', labelsize=6)
    for pane in (ax.xaxis.pane, ax.yaxis.pane, ax.zaxis.pane):
        pane.set_facecolor((0.97, 0.97, 0.97, 0.15))
        pane.set_edgecolor('#dddddd')
    ax.xaxis._axinfo['grid'].update(color='#dddddd', linewidth=0.4, linestyle='--')
    ax.yaxis._axinfo['grid'].update(color='#dddddd', linewidth=0.4, linestyle='--')
    ax.zaxis._axinfo['grid'].update(color='#dddddd', linewidth=0.4, linestyle='--')
...
for i, (noise, sig, ax) in enumerate(zip(noise_cones, sig_cones, axes.flat)):
    ...
    ax.imshow(matrix, aspect='auto', cmap='hot', vmax=3, origin='lower')
    ax.tick_params(labelsize=6)
plt.tight_layout()
plt.show()

# ================================================================================
# ================= PLOT 4a — Single signal r-z matrix ===========================

_sig_cone_idx = next(i for i, s in enumerate(sig_cones) if len(s) > 0)
_matrix = make_rz_matrix(noise_cones[_sig_cone_idx], sig_cones[_sig_cone_idx],
                         r_bins=100, z_bins=100)
fig, ax = plt.subplots(figsize=(6, 5))
ax.imshow(_matrix, aspect='auto', cmap='hot', vmax=3, origin='lower')
ax.axis('off')
plt.tight_layout()
plt.show()

# ================================================================================
# ================= PLOT 4a2 — Augmentation: 2 signal + 2 noise realisations =====
# Two different noise backgrounds sampled from the same cone hit pool,
# so the backgrounds genuinely differ between realisations.

_cone_all   = hits_cones[_sig_cone_idx].copy()
_cone_sig   = sig_cones[_sig_cone_idx]
_noise_pool = _cone_all[~_cone_all['hit_id'].isin(_cone_sig['hit_id'])]

_noise1 = _noise_pool.sample(frac=0.01, random_state=0)
_noise2 = _noise_pool.sample(frac=0.01, random_state=7)
_empty  = _cone_sig.iloc[0:0]

_panels = [
    (_noise1, _cone_sig, 'Signal, realisation 1'),
    (_noise2, _cone_sig, 'Signal, realisation 2'),
    (_noise1, _empty,    'Noise, realisation 1'),
    (_noise2, _empty,    'Noise, realisation 2'),
]

fig, axes = plt.subplots(2, 2, figsize=(10, 8))
for ax, (noi, sig, title) in zip(axes.flat, _panels):
    m = make_rz_matrix(noi, sig, r_bins=100, z_bins=100)
    ax.imshow(m, aspect='auto', cmap='hot', vmax=3, origin='lower')
    ax.set_title(title, fontsize=9)
    ax.set_xlabel('z bins', fontsize=8)
    ax.set_ylabel('r bins', fontsize=8)
...
gs  = fig.add_gridspec(2, 1, height_ratios=[10, 1], hspace=0.04)
...
ax_occ.imshow(_occ, aspect='auto', origin='lower', cmap='plasma', vmin=0, vmax=1)
ax_occ.axis('off')
...
# ================================================================================
# ================= PLOT 4e — r-z and r-phi side by side (same signal cone) ======

# pick the signal cone whose stripe sits furthest to the left in r-phi
def _phi_norm(i):
    sx, sy, _ = cone_signs_list[i]
    phi_min, phi_max = CONE_PHI_RANGES[(sx, sy)]
    return (sig_cones[i]["phi"].mean() - phi_min) / (phi_max - phi_min)

_se_idx = min((i for i, s in enumerate(sig_cones) if len(s) > 0), key=_phi_norm)
_se_sx, _se_sy, _ = cone_signs_list[_se_idx]
_rz_matrix  = make_rz_matrix(noise_cones[_se_idx], sig_cones[_se_idx], r_bins=100, z_bins=100)
_rp_full    = make_rphi_extended_matrix(noise_cones[_se_idx], sig_cones[_se_idx], _se_sx, _se_sy)
_rp_binary  = _rp_full[:SNN_R_BINS]
_rp_occ     = _rp_full[SNN_R_BINS:SNN_R_BINS + 1]

fig = plt.figure(figsize=(14, 5))
gs  = fig.add_gridspec(2, 2, height_ratios=[10, 1], hspace=0.04, wspace=0.08)

ax_rz   = fig.add_subplot(gs[:, 0])
ax_rphi = fig.add_subplot(gs[0, 1])
ax_occ  = fig.add_subplot(gs[1, 1])

ax_rz.imshow(_rz_matrix,  aspect='auto', cmap='hot', vmax=3, origin='lower')
ax_rz.axis('off')
ax_rz.set_title('r–z', fontsize=10)

ax_rphi.imshow(_rp_binary, aspect='auto', origin='lower', cmap='hot', vmax=3)
ax_rphi.axis('off')
ax_rphi.set_title('r–phi', fontsize=10)

ax_occ.imshow(_rp_occ, aspect='auto', origin='lower', cmap='plasma', vmin=0, vmax=1)
ax_occ.axis('off')
...
# # ================================================================================
# # ================= PLOT 5 — r-phi spike raster (100-bin, 1% noise) ==============

# _rphi_path = os.path.join(DIR_100, "train", "X_rphi.npy")
# if os.path.exists(_rphi_path):
#     _X_rphi  = np.load(_rphi_path)
#     _y_s1    = np.load(os.path.join(DIR_100, "train", "y_stage1.npy"))

#     _sig_idx = np.where(_y_s1 == 1)[0]
#     _noi_idx = np.where(_y_s1 == 0)[0]
#     np.random.seed(0)
#     _show = [
#         np.random.choice(_sig_idx),
#         np.random.choice(_sig_idx),
#         np.random.choice(_noi_idx),
#         np.random.choice(_noi_idx),
#     ]

#     R_SHOW = 25
#     fig, axes = plt.subplots(1, 4, figsize=(14, 4), sharey=True)
#     for ax, idx in zip(axes, _show):
#         mat = _X_rphi[idx]
#         for r_bin in range(R_SHOW + 1):
#             ax.hlines(r_bin, 0, mat.shape[1] - 1, color='black', lw=0.4, alpha=0.35)
#             phi_hits = np.where(mat[r_bin] > 0)[0]
#             if len(phi_hits):
#                 ax.vlines(phi_hits, r_bin - 0.7, r_bin, color='black', lw=0.9)
#         label = 'Signal' if _y_s1[idx] == 1 else 'Noise'
#         ax.set_xlabel('phi bin')
#         ax.set_xlim(0, mat.shape[1] - 1)
#         ax.set_ylim(R_SHOW + 0.5, -1)
#         ax.axis('off')
#     axes[0].set_ylabel('r bin')
#     plt.tight_layout()
#     plt.show()
# else:
#     print(f"r-phi matrix not found: {_rphi_path}")

# # ================================================================================
# # ================= PLOT 6 — r-z spike raster (100-bin, 1% noise, one signal) ===

R_SHOW = 25
mat    = _matrix  # same cone as Plot 4a
fig, ax = plt.subplots(figsize=(7, 5))
for r_bin in range(R_SHOW + 1):
    ax.hlines(r_bin, 0, mat.shape[1] - 1, color='black', lw=0.4, alpha=0.35)
    z_hits = np.where(mat[r_bin] > 0)[0]
    if len(z_hits):
        ax.vlines(z_hits, r_bin, r_bin + 0.7, color='black', lw=0.9)
ax.set_xlim(0, mat.shape[1] - 1)
ax.set_ylim(-1, R_SHOW + 0.5)
ax.axis('off')
plt.tight_layout()
plt.show()
...
# ================================================================================
# ================= PLOT 7 — Stage 1 training curves (100-event model) ===========
# Model : matrices100/model_stage1.pt
# Data  : matrices100/train  +  matrices100/val
...
if os.path.exists(S1_100_PATH):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'loss_stage1_100events.pdf'), bbox_inches='tight')
    plt.show()
else:
    print(f"Stage 1 (100-event) model not found: {S1_100_PATH}")

# ================================================================================
# ================= PLOT 8 — Stage 1 training curves (1000-event model) ==========
# Model : matrices1000/model_stage1.pt
# Data  : matrices1000/train  +  matrices1000/val

S1_1000_PATH = os.path.join(DIR_1000, "model_stage1.pt")

if os.path.exists(S1_1000_PATH):
    ckpt         = torch.load(S1_1000_PATH, map_location='cpu')
    train_losses = ckpt['train_losses']
    val_losses   = ckpt['val_losses']
    val_accs     = ckpt['val_accs']
    epochs       = range(1, len(train_losses) + 1)
    ...
    print(f"Stage 1 (1000-event) model not found: {S1_1000_PATH}")

# ================================================================================
# ================= PLOT 8b — Stage 1 training curves (150-bin, 100-event model) =
# Model : matrices150/model_stage1.pt
# Data  : matrices150/train  +  matrices150/val

S1_150_PATH = os.path.join(DIR_150, "model_stage1.pt")
...
if os.path.exists(S1_150_PATH):
    ...
    print(f"Stage 1 (150-bin 100-event) model not found: {S1_150_PATH}")

# ================================================================================
# ================= PLOT 9 — Stage 2 training curves (1000-event model) ==========
# Model : matrices1000/stage2_s2_h512_256_lr1e-3_dr0.1_pw10.pt

S2_1000_PATH = os.path.join(DIR_1000, "stage2_s2_h512_256_lr1e-3_dr0.1_pw10.pt")

if os.path.exists(S2_1000_PATH):
    ckpt         = torch.load(S2_1000_PATH, map_location='cpu')
    train_losses = ckpt['train_losses']
    val_losses   = ckpt['val_losses']
    ...
    print(f"Stage 2 (1000-event) model not found: {S2_1000_PATH}")
...
model_dir = r"C:\Users\felix\Downloads\Skola\Kand\BACHELOR_CODE\data\matrices150"

# ── Load all models ──────────────────────────────────────────────────────────
results = []
for f in sorted(os.listdir(model_dir)):
    if not f.endswith(".pt") or f == "model_stage1.pt":
        continue
    path = os.path.join(model_dir, f)
    cp        = torch.load(path, map_location='cpu')
    val_losses = cp['val_losses']
    best_val   = min(val_losses)
    best_ep    = val_losses.index(best_val) + 1
    cfg        = cp['config']
    label      = f"h{cfg['H1']}_{cfg['H2']} lr={cfg['LR']} dr={cfg['DROPOUT']}"
    results.append((best_val, best_ep, label, val_losses, cp['train_losses']))

results.sort()  # sort by best val loss

# ── Print ranking ────────────────────────────────────────────────────────────
print(f"\n{'Rank':<5} {'Best val':>9} {'Epoch':>6}  Config")
print("-" * 60)
for rank, (best_val, best_ep, label, val_losses, _) in enumerate(results, 1):
    print(f"{rank:<5} {best_val:>9.4f} {best_ep:>6}  {label}")

# ── Plot all val loss curves ──────────────────────────────────────────────────
fig, axes = plt.subplots(4, 6, figsize=(20, 12))
fig.subplots_adjust(hspace=0.5, wspace=0.4)

for ax, (best_val, best_ep, label, val_losses, train_losses) in \
        zip(axes.flat, results):
    epochs = range(1, len(val_losses) + 1)
    ax.plot(epochs, train_losses, color='#0077BB', lw=1.0, label='train')
    ax.plot(epochs, val_losses,   color='#EE7733', lw=1.0, label='val')
    ax.axvline(best_ep, color='grey', lw=0.8, ls='--', alpha=0.7)
    ax.set_title(label, fontsize=6, pad=3)
    ax.set_ylim(0.55, 1.5)
    ax.tick_params(labelsize=5)

# highlight winner
for spine in axes.flat[0].spines.values():
    spine.set_edgecolor('green')
    spine.set_linewidth(2.0)

axes.flat[0].set_title(results[0][2] + "\n★ WINNER", fontsize=6,
                        color='green', pad=3)

# shared legend
handles = [plt.Line2D([0], [0], color='#0077BB', label='train'),
           plt.Line2D([0], [0], color='#EE7733', label='val')]
fig.legend(handles=handles, loc='lower center', ncol=2,
           fontsize=9, bbox_to_anchor=(0.5, -0.02))

fig.suptitle("Grid search — validation loss for all 24 configurations "
             "(sorted by best val loss, winner top-left)",
             fontsize=11, y=1.01)
